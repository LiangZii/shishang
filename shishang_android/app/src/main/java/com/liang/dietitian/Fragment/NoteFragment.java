...
import com.liang.dietitian.adapter.NoteAdapter;
...
public class NoteFragment extends Fragment {
    private View view;
    private SwipeRecyclerView noteList;
    ...
    private void initView() {
        ...
        //        设置监听器与绑定菜单
        noteList.setSwipeMenuCreator(new SwipeMenuCreator() {
            @Override
...
//  菜单单击的监听器
        noteList.setOnItemMenuClickListener(new OnItemMenuClickListener() {
            @Override
...
//                historyBrowserDataBase.deleteData(sqLiteDatabase, adapter.getItem(adapterPosition).getToDetailSrc());
        ...
        noteList.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(View view, int adapterPosition) {
                ...
                bundle.putSerializable("note", adapter.getItem(adapterPosition));
...
//                  子线程更新UI
                    ...
                    getActivity().runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                List<Note> notes = JSON.parseArray(data, Note.class);
                                if (code == 200) {
                                    adapter.clearNote();
                                    for (Note n : notes) {
                                        adapter.addNote(n);
    ...
    public void doRequestForDelete(Integer position) {
        ...
        Request request = new Request.Builder()
                .url(Config.UrlHeader + "/notes/" + adapter.getItem(position).getNoteId())
                .addHeader("token", token)
                .delete()
                .build();
